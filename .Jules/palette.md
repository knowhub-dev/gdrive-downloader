...
## 2026-05-15 - [Semantic Input Types in Themed UIs]
**Learning:** Updating input types (like `type="text"` to `type="url"`) improves mobile UX by triggering appropriate keyboards, but it can break themed UI designs if existing CSS selectors are strictly bound to `type="text"`.
**Action:** Always verify and update corresponding CSS selectors (e.g., adding `input[type="url"]` alongside `input[type="text"]`) when introducing new semantic HTML5 input types to preserve visual consistency across the application.
