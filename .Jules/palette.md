...
## 2026-06-25 - [Semantic Input for Mobile UX]
**Learning:** Using semantic HTML5 input types (like `type="url"` or `type="email"`) drastically improves the mobile experience by triggering the correct keyboard layout, reducing friction during data entry.
**Action:** When working on web forms, especially those accessed on mobile devices, replace generic `type="text"` with semantically appropriate types, ensuring that existing CSS styles are updated to target these new types to maintain visual consistency.
