...
## 2026-04-06 - [Semantic Input Types for Mobile UX]
**Learning:** Using semantic HTML5 input types (like `type="url"`) significantly improves mobile user experience by triggering context-appropriate virtual keyboards (e.g., showing ".com" or "/" keys).
**Action:** When creating forms, always use the most specific semantic input type (`email`, `url`, `tel`, etc.) instead of generic text inputs to ensure optimal mobile accessibility and UX, and remember to update corresponding CSS selectors to maintain consistent styling.
